 ...
 ******************************************************************************/
  ...
  A *= sqrt(lambda + n_x);
  for (int i = 0; i < n_x; ++i) {
    Xsig.col(1 + i      ) = x + A.col(i);
    Xsig.col(1 + i + n_x) = x - A.col(i);
  }
 ...
 ******************************************************************************/
  ...
  for (int i=0; i < n_aug; ++i) {
    ...
    Xsig_aug.col(1+i+n_aug) = x_aug - A.col(i);
  }
 ...
 ******************************************************************************/
  ...
  Tc *= 0.0;
  for (int i = 0; i < n_sigma; ++i) {
    diff_state = Xsig_pred.col(i) - x;
    diff_meas = Zsig.col(i) - z_pred;
    Tc += weights(i)*diff_state*diff_meas.transpose();
  }
